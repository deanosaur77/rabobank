...
@ActiveProfiles("test")
class DebitCardControllerIntegrationTest {

	private static final String NO_CARD = "NO CARD";

	private static final String DEBIT_CARD = "60111";
	...
	public void test_withdraw_success() throws Exception {
		...
		request.setAccountNumber(DEBIT_CARD);
		request.setAmount(2500.00);

		AccountResponse response = restTemplate.postForObject("/api/debit-card/withdraw", request, AccountResponse.class);
		List<TransactionDetail> transactionDetails = response.getTransactionDetail();

		assertEquals(DEBIT_CARD, response.getAccountNumber());
		assertEquals(Status.SUCCESS.toString(), response.getStatus());
		assertEquals(AccountType.DEBIT_CARD.toString(), response.getAccountType());
		assertEquals(7500.00, response.getBalance(), 0);
		...
		assertEquals(10000, transactionDetails.get(0).getPreviousBalance(), 0);
	}
	...
	public void test_deposit_success() throws Exception {
		...
		AccountResponse response = restTemplate.postForObject("/api/debit-card/deposit", request, AccountResponse.class);
		List<TransactionDetail> transactionDetails = response.getTransactionDetail();

		assertEquals(DEBIT_CARD, response.getAccountNumber());
		assertEquals(Status.SUCCESS.toString(), response.getStatus());
		assertEquals(AccountType.DEBIT_CARD.toString(), response.getAccountType());
		assertEquals(15000.00, response.getBalance(), 0);
	...
	public void test_withdraw_returns_insufficient_balance() throws Exception {
		...
		assertEquals("Debit Card has insufficient balance for withdrawel." , response.getMessage());
	}
	...
	public void test_withdraw_returns_account_not_found() throws Exception {
		...
		assertEquals(Status.ERROR.toString(), response.getStatus());
		assertEquals("Debit Card account not found for withdrawel." , response.getMessage());
	}
	...
	public void test_deposit_returns_account_not_found() throws Exception {
		...
		assertEquals(NO_CARD, response.getAccountNumber());
		assertEquals(Status.ERROR.toString(), response.getStatus());
		assertEquals("Debit Card account not found for deposit." , response.getMessage());
	}
